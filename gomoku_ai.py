...
def main():
    ...
    while True:
        ...
        if turn == BLACK:
            ...
            # c1, p1 = ai.find_max(weight.board, turn)
            # c2, p2 = ai.find_max(weight.board, after)
            # if c1 > c2:
            #     y, x = p1
            # else:
            #     y, x = p2
            ...
            # else:
            #     y, x = p2
            _, position = ai.minimax(board.board, weight.board, 5, turn, after, -INF, INF)
            if position == (-1, -1):
                print('minimax error')
                return
            y, x = position
        ...
        if board.is_win(turn) or board.is_full():
            print('game is end')
            return

        board.draw()

        weight.board[turn-1][y][x] = -turn
        weight.board[after-1][y][x] = -turn

        weight.calc_weight(turn)
        weight.calc_weight(after)
